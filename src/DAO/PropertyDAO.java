...
public class PropertyDAO {
    ...
    public boolean updateProperty(Property p) {
        ...
        String query = "UPDATE properties SET title=?, address=?, property_type=?, bedrooms=?, bathrooms=?, monthly_rent=?, deposit=?, available_from=?, image_path=? WHERE property_id=? AND owner_id=?";
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            if (p.getAvailableFrom() != null) {
                stmt.setDate(8, new java.sql.Date(p.getAvailableFrom().getTime()));
            } else {
                stmt.setNull(8, java.sql.Types.DATE);
            }
            stmt.setString(9, p.getPrimaryImagePath());
            stmt.setInt(10, p.getPropertyId());
            stmt.setInt(11, p.getOwnerId());
            return stmt.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
        }
